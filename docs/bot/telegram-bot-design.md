...
| `/tomorrow` | DM & Groups | DM: `Показати розклад на завтра`<br>Group: `Показати розклад групи на завтра` | ✅ Implemented | In DMs: shows tomorrow's personal classes. In groups: shows tomorrow's overall group schedule. |
| `/week` | DM & Groups | DM: `Показати розклад на тиждень`<br>Group: `Показати розклад групи на тиждень` | ✅ Implemented | In DMs: shows personal week schedule. In groups: shows overall group schedule for a week. |
...
- **Command Scoping**: Telegram command lists are registered via `setMyCommands` using:
  ...
  - `BotCommandScopeAllGroupChats`: group schedule (`/today`, `/tomorrow`, `/week`), personal schedule in group (`/me_today`, `/me_tomorrow`, `/me_week`), `/group_url_sync`.
...
- **Enforced Isolation**: Commands designed for DMs only (`/link`, `/urls`) return `⚠️ Ця команда доступна лише в особистих повідомленнях з ботом.` if invoked in a group. Conversely, group-only commands return `⚠️ Ця команда доступна лише у групових чатах.` if invoked in DMs.
- **Hyphen & Underscore Aliases**: Group and personal schedule commands are registered in Telegram with underscores (`/today`, `/me_today`, `/group_today`, etc.), while message parsing also transparently supports hyphenated aliases (`/group-today`, `/group-tomorrow`, `/group-week`, `/me-today`, `/me-tomorrow`, `/me-week`).
...
#### 3. Caller Attribution on `/me_today`, `/me_tomorrow`, and `/me_week` in Groups
- When `/me_today`, `/me_tomorrow`, or `/me_week` is invoked in a group chat, it fetches the personalized schedule of the invoking user and prefixes the message with:
...
#### 4. Group Schedules (`/today`, `/tomorrow` & `/week` in Groups)
- In group chats, `/today`, `/tomorrow`, and `/week` fetch and display the official group schedule directly from `api.campus.kpi.ua` for the academic group bound to that Telegram chat.
- Inline navigation buttons (`gnav:` and `gweek:`) navigate dates and weeks for the entire group.
- Legacy aliases `/group_today`, `/group_tomorrow`, and `/group_week` remain fully functional.
...
- Once configured, group schedule messages (`/today`, `/tomorrow`, `/week`, `/group_today`, `/group_tomorrow`, and `/group_week`) render clickable `[Онлайн]` links pointing to the configured meetings.
...
Every button in the bot — onboarding, day navigation, week navigation — **edits the existing message in place** rather than sending a new one, so the chat is never flooded. Only typed commands (`/start`, `/link`, `/today`, `/tomorrow`, `/week`, `/me_today`, `/me_tomorrow`, `/me_week`, `/group_today`, `/group_tomorrow`, `/group_week`) post a new message, since there is nothing on screen to edit yet. This uses:
