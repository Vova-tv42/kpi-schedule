...
func TestDispatchTest(t *testing.T) {
	ctx := context.Background()
	dbPath := filepath.Join(t.TempDir(), "test_alerts_dispatch_test.db")
	if err := storage.Migrate(dbPath); err != nil {
		t.Fatalf("migrate: %v", err)
	}
	...
	_, err = db.UpsertUser(ctx, 333444, nil, nil)
	if err != nil {
		t.Fatalf("upsert user: %v", err)
	}
	...
	testTime := time.Date(2026, 9, 4, 21, 55, 0, 0, loc)
	res, err := d.DispatchTest(ctx, testTime)
	if err != nil {
		t.Fatalf("dispatch test: %v", err)
	}
	if res.PersonalAlertsSent != 1 {
		t.Fatalf("expected 1 personal test alert sent, got %d", res.PersonalAlertsSent)
	}
	if len(sender.sentMessages) != 1 {
		t.Fatalf("expected 1 sent message, got %d", len(sender.sentMessages))
	}
	msg := sender.sentMessages[0]
	if msg.ChatID != 333444 {
		t.Errorf("expected chatID 333444, got %d", msg.ChatID)
	}
	if !strings.Contains(msg.Text, "<blockquote>🔔 Тестове сповіщення від крону</blockquote>") {
		t.Errorf("expected blockquote header, got: %s", msg.Text)
	}
	if !strings.Contains(msg.Text, "<code>21:55</code>") {
		t.Errorf("expected time 21:55, got: %s", msg.Text)
	}

	// Resend immediately to verify it resends regardless of time/dedup
	res2, err := d.DispatchTest(ctx, testTime)
	if err != nil {
		t.Fatalf("dispatch test resend: %v", err)
	}
	if res2.PersonalAlertsSent != 1 || len(sender.sentMessages) != 2 {
		t.Errorf("expected resend to succeed, got %d (total: %d)", res2.PersonalAlertsSent, len(sender.sentMessages))
	}
}
